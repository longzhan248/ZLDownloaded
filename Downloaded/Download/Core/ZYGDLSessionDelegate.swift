...
extension ZYGDLSessionDelegate: URLSessionDownloadDelegate {
    ...
    public func urlSession(_ session: URLSession,
        ...
        if let task = downloadTask.zygdlTask {
            task.didWriteData(
                downloadTask: downloadTask, bytesWritten: bytesWritten,
            ...
            guard let task = manager.mapTask(currentURL) else {
                manager.log(.error("urlSession(_:downloadTask:didWriteData:totalBytesWritten:totalBytesExpectedToWrite:)",error: ZYGDLError.fetchDownloadTaskFailed(url: currentURL)))
                return
            }
            downloadTask.zygdlTask = task
            task.didWriteData(
                downloadTask: downloadTask, bytesWritten: bytesWritten,
                totalBytesWritten: totalBytesWritten,
                totalBytesExpectedToWrite: totalBytesExpectedToWrite)
    ...
    public func urlSession(_ session: URLSession,
        ...
        if let downloadTask = task.zygdlTask {
